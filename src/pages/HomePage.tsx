...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-zinc-950 text-zinc-100 flex flex-col items-center">
      {/* Hero Section */}
      <section className="relative w-full max-w-6xl px-6 pt-20 pb-16 flex flex-col items-center text-center">
        {/* Background glow gradient */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[350px] bg-violet-600/15 rounded-full blur-3xl pointer-events-none -z-10" />

        {/* Announcement Tag */}
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-violet-500/30 bg-violet-500/10 text-xs font-medium text-violet-300 mb-8 backdrop-blur-md">
          <Sparkles className="w-3.5 h-3.5 text-violet-400" />
          <span>Next-Generation Theory of Computation Tool</span>
        </div>

        {/* Hero Title */}
        <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-zinc-100 max-w-4xl leading-[1.1]">
          AutomataLab
        </h1>

        {/* Subtitle */}
        <p className="mt-4 text-xl sm:text-2xl font-medium text-violet-400 tracking-tight">
        ...
        <div className="mt-16 w-full max-w-4xl rounded-2xl border border-zinc-800/80 bg-zinc-900/60 p-6 shadow-2xl backdrop-blur-xl">
          <div className="flex items-center justify-between border-b border-zinc-800 pb-4 mb-6">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-rose-500/80" />
              <div className="w-3 h-3 rounded-full bg-amber-500/80" />
              <div className="w-3 h-3 rounded-full bg-emerald-500/80" />
              <span className="ml-2 font-mono text-xs text-zinc-400">DFA Model: M = (Q, Σ, δ, q₀, F)</span>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-around gap-6 py-4">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="relative flex items-center justify-center w-14 h-14 rounded-full border-2 border-violet-400 bg-violet-950/60 font-mono font-bold text-violet-200">
                <span className="absolute -left-10 text-[10px] text-violet-400 font-mono flex items-center">Start →</span>
                q0
              </div>
            ...
            <div className="flex flex-col items-center">
              <div className="font-mono text-xs font-bold text-zinc-300 bg-zinc-800 px-3 py-1 rounded-full border border-zinc-700">
                δ(q0, 0) → q1
              </div>
              <ArrowRight className="w-6 h-6 text-zinc-600 my-1" />
            </div>

            {/* Visual Mini Mock Node q1 */}
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center justify-center w-14 h-14 rounded-full border-2 border-zinc-500 bg-zinc-900 font-mono font-bold text-zinc-200">
                q1
              </div>
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="relative flex items-center justify-center w-14 h-14 rounded-full border-2 border-emerald-400 bg-emerald-950/40 font-mono font-bold text-emerald-200">
                <div className="absolute w-11 h-11 rounded-full border-2 border-emerald-400" />
                q2
              </div>
              <span className="text-[11px] text-emerald-400 font-mono">Accepting State (F)</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Feature Cards Grid */}
      <section className="w-full max-w-6xl px-6 py-16 border-t border-zinc-800/80">
